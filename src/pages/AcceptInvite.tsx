...
const AcceptInvite = () => {
  ...
  const whatsappUrl = getWhatsAppUrl(
    "Hola, mi link de invitación a Prime F&H no funciona. ¿Me pueden ayudar a entrar?"
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Cuenta activada. ¡Bienvenido a Prime F&H!");
      // El redirect se decide por el rol real, mismo criterio que LoginDual.tsx
      navigate(response.data.user.role === "patient" ? "/app/dashboard" : "/app/admin");
    } catch (error: any) {
      // Solo el código específico del backend significa "el link ya no
      // sirve". Cualquier otro error (contraseña corta, un 500 puntual, sin
      // red) se queda en el formulario con un aviso, sin descartar lo que la
      // persona ya escribió.
      if (error.response?.data?.code === "INVITE_EXPIRED") {
        setInviteInvalid(true);
      } else {
        toast.error(error.response?.data?.message || "No se pudo activar la cuenta. Intenta de nuevo.");
      }
    } finally {
      setIsLoading(false);
    }
        ...
        >
          ...
          <Card className="p-6 bg-white/95 backdrop-blur-sm border-0 shadow-2xl">
            ...
            <p className="text-center text-gray-700 mb-2">
              Tu invitación ya fue usada o expiró (los links duran 7 días).
            </p>
            <p className="text-center text-sm text-gray-500 mb-6">
              Contacta a Prime F&H para que te reenvíen la invitación.
            </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-dark via-brand-dark/95 to-brand-secondary/20 flex items-center justify-center p-4">
      ...
      >
        <div className="text-center mb-6">
          ...
          <p className="text-brand-light/70 text-sm mt-1">
            Activa tu acceso al portal de Prime F&H
          </p>
        </div>
